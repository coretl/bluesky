...
- ``RunEngineResult.exception`` is an exception instance for all three of
  ``stop``, ``abort`` and ``halt``.  It was an instance for ``abort`` and the
  class for the other two, which made ``isinstance`` answer for one verb and
  not the others; code telling the three apart should compare the type.  It is
  annotated ``BaseException | None`` rather than ``Exception | None``, because
  ``halt`` throws in a ``PlanHalt``, which is a ``GeneratorExit``.
