...
.. warning::
   ...
   The API on this page is provisional. It is being reviewed against the needs
   of the services that will consume it, and names, signatures and behaviour
   may change without a deprecation period. Code written against it should
   expect to be revisited; `RunEngine` itself is not provisional, and the
   changes described here do not reach it.
...
Stopping a plan is not failing it, which is the next section's subject.

What awaiting a runner does
---------------------------

``await runner`` waits for the plan and gives back what the plan returned. It
is not `RunEngine.__call__` with the blocking removed, and the difference is
worth stating outright, because a `RunEngine` turns three of the four endings
into exceptions and a runner turns only one:

**The plan ran to its end.** The await gives its return value. This is the only
case the two agree on entirely.

**The plan raised.** The exception comes out of the await, as above. A
`RunEngine` raises the same exception from ``__call__``.

**The plan was stopped, aborted or halted.** The await *returns*, and what it
returns is :data:`~bluesky.plan_runner.NO_PLAN_RETURN` -- the plan did not
reach its own ``return``, and a runner will not invent a value for it. Nothing
is raised. A `RunEngine` raises
:class:`~bluesky.utils.RunEngineInterrupted` here, because at a prompt an
interruption has to stop the caller; a service reads ``interrupted`` instead:
...
**The plan is paused.** The await does not return and does not raise -- it goes
on waiting, because a paused plan has not ended. It completes when something
resumes the plan, or ends it. This is the ending with no `RunEngine`
counterpart at all: ``RE(plan)`` returns control to the prompt by raising
``RunEngineInterrupted``, precisely so a human can type ``RE.resume()``. A
service that awaits a runner and pauses it from a different task must arrange
the resume from that other task; awaiting is not how you find out a plan
paused. ``hooks.state`` is, and ``runner.done()`` answers it after the fact.

Awaiting twice is allowed and gives the same answer twice. The plan ran once,
and the runner is the record of it.
