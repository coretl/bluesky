...
from ._loop import call_soon_or_now, running_on
...
class SuspenderBase(metaclass=ABCMeta):
    ...
    def install(self, suspension: Suspension, *, event_type: str | None = None) -> None:
        ...
        Call on the suspension's loop; `RunEngine.install_suspender` does that
        for you.
    ...
    def remove(self) -> None:
        ...
        Call on the suspension's loop; `RunEngine.remove_suspender` does that
        for you.
        """
    ...
    def __decide(self, value) -> None:
        """Trip or clear the suspension for ``value``. On the loop."""
        suspension = self._suspension
        if suspension is None:
        ...
        if self._should_suspend(value):
            ...
            # Only clear what this tripped, or a pending clear could drop a later trip.
            self._tripped = False
