...
def _md(md, RE, hw):
    ...
    scan = simple_scan(hw.motor)
    RE(scan, {"start": [validate_dict_cb("project", "standing")]}, project="standing")
...
def test_failed_status_object(RE):
    ...
    with pytest.raises(FailedStatus):
        RE([Msg("set", ff, None, group="a"), Msg("wait", None, group="a")])

    with pytest.raises(FailedStatus):
        RE([Msg("trigger", ff, group="a"), Msg("wait", None, group="a")])
...
def test_sync_trigger_delay(hw):
    ...
    hw.motor.delay = 0.5
    hw.det.exposure_time = 0.5
    assert hw.det.exposure_time == 0.5

    _time_test(hw.motor.set, 0.5, 1)
    _time_test(hw.det.trigger, 0.5)
