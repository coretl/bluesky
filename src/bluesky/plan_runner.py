...
class PlanRunner:
    ...
    .. warning::
       ...
       This API is provisional. It is being reviewed against the needs of the
       services that will consume it, and names, signatures and behaviour may
       change without a deprecation period. `RunEngine`, which is built on it,
       is not provisional and is unaffected.
    ...
    holds no locks and no threading primitives. Emission is the one thing that
    does not: a sync ophyd signal fires its monitor callback on the device's own
    thread, and that reaches the dispatcher directly, so a document can be given
    to subscribers off the loop. See `bluesky.dispatcher.Dispatcher.process`.
    ...
    interrupted : bool
        Whether the plan was paused, stopped, aborted or halted. False for a
        plan that ran to its end, whether it succeeded or raised.
    exit_status : str
        How the plan ended: ``'success'``, ``'abort'`` or ``'fail'``. Optimistic
        until it ends, and recorded on the RunStop of every run still open.
    exit_reason : str
        Why, in words. What was passed to :meth:`stop`, or the string of the
        exception the plan raised, which wins because it happens later.
    exit_exception : BaseException or None
        What was thrown in to end the plan early: `~bluesky.utils.RequestStop`,
        `~bluesky.utils.RequestAbort` or `~bluesky.utils.PlanHalt`. Only a
        paused plan has one -- a running one is cancelled instead, and there is
        nothing to record. Not the exception a *plan* raised: that one reaches
        whoever awaited this runner.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # The outcome, documented in the class docstring. Public because
        # whoever ran the plan has to be able to read it: a `RunEngine` puts
        # these in the `RunEngineResult` it returns, and a headless caller has
        # nothing else to ask. `exit_exception` is separate from `_exception`,
        # which is the transport -- the run loop takes that one and clears it on
        # the way into the plan, routinely before the caller that ended the plan
        # can look.
    ...
    async def stop(self, *, success: bool = True, finalize: bool = True, reason: str = "") -> None:
        ...
        self.interrupted = True
        # Recorded here rather than left for the run loop: the clauses
        # there set it from an exception, and one raised on the way out
        # of a stopped plan is the better answer for why it ended.
        ...
        if was_paused:
            ...
            # An instance for all three verbs, so that the three ends of a plan
            # are told apart by which exception it is and not by whether it
            # happens to have been built. `main` recorded an instance for abort
            # and the class for stop and halt; that is a wart of how each verb
            # raised, not something a caller can use. Only the record:
            # `_exception` stays the class it already was, because the run loop
            # compares it by identity with what comes back out of the plan.
