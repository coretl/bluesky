...
from ._loop import call_soon_or_now, run_coro_on_loop
...
from .suspension import Suspension, SuspensionReason, join_justifications
...
class RunEngine:
    ...
    def __init__(
        ...
        *,
        ...
        self._held = False
        # Suspends the plan when the suspension trips. Rebuilt per plan and per resume.
    ...
    def _arrange_suspension(self, at_start: bool):
        """Hold the plan while the suspension is tripped, and restart the supervisor.
        ...
        if tripped:
            self._announce_held(reasons, at_start)
        ...
        if tripped and not self._held:
            ...
            self._plan_stack.append(self._holding(self._hold()))
            self._response_stack.append(None)
    ...
    async def _supervise_suspension(self, tripped_at_start=False):
        ...
        """
        if tripped_at_start:
            # Held in band by `_arrange_suspension`; wait for that to clear.
            await self._suspension.wait_cleared()
        while True:
            # Nothing while paused or pausing: `resume` restarts this task.
            while not (reasons := self._suspension.reasons) or self._state in ("paused", "pausing"):
    ...
    def _begin_suspension(self, opening):
        ...
        self._response_stack.append(None)
        # Not from 'paused': `resume` replays onto the pushed message.
    ...
    async def _run(self):
        ...
        try:
            ...
            # Stop the supervisor; the next plan or resume starts another.
            if self._supervisor is not None:
                self._supervisor.cancel()
