...
class RunEngine:
    ...
    def __init__(
        ...
        *,
        ...
        # Permission to run. Suspenders installed here withhold it; the plan
        # waits for it before its first message, and is suspended if it is
        # withheld after that.
        self._permit = Permit("RunEngine", loop)
    ...
    def __call__(
        ...
        **metadata_kw: typing.Any,
        ...
        if suspension is not None:
            # A wait, not a suspension: there is no checkpoint to rewind to
            # before the plan's first message, so all that can be done is hold.
    ...
    def permit(self):
        ...
        Install a suspender onto this to have it hold up every plan this engine
        runs, which is what :meth:`install_suspender` does.
        """
        return self._permit
    ...
    def _suspend_until(self, fut, *, pre_plan=None, post_plan=None, justification=None):
        ...
        async def _request_suspend(pre_plan, post_plan, justification):
            if not self.resumable:
                ...
                if not was_paused:
                    self._task.cancel()
                # Nothing to push a suspension onto: the plan stack is being
                # torn down.
                return
