...
from .suspensions import Suspension, join_justifications, running_on
...
class RunEngine:
    ...
    def __init__(
        ...
        *,
        ...
        # What the suspenders trip, and what a held plan waits on. One per
        # engine: two conditions tripping at once are two reasons on this
        # one suspension, not two suspensions.
        self._suspension = Suspension("engine", self._loop)
        # Set when the plan comes to rest paused; see the pause block in
        # `_run` and `_arrange_suspension`.
        self._cleared_when_paused = True
        # Watches that suspension and puts a suspension in front of the
        # plan when it trips. Rebuilt per plan and per resume.
    ...
    def __call__(
        ...
        **metadata_kw: typing.Any,
        ...
        # After the plan is on the stack, so that anything this pushes to hold
        # the plan goes in front of it.
        self._arrange_suspension()
    ...
    def resume(self):
        ...
        self._response_stack.append(None)
        # A condition can go bad while the plan is paused, and the supervisor
        # arranges nothing while it is: re-read the suspension on the way back
        # in, so a trip that happened during the pause holds the plan now.
        self._arrange_suspension()
    ...
    def _on_loop(self, func, *args):
        """Run ``func(*args)`` on the event loop, and wait for it.

        A suspension is written only on the loop that owns it, so the calls
        that write one have to get there. Called from the loop itself -- a
        message handler, say -- it runs inline rather than scheduling work onto
        a loop it is already running on and waiting for it, which deadlocks.
        """
        ...
        def run_it():
            try:
                done.set_result(func(*args))
            except BaseException as exc:  # noqa: BLE001
                done.set_exception(exc)

        self._loop.call_soon_threadsafe(run_it)
        return done.result()
    ...
    def install_suspender(self, suspender):
        ...
        self._on_loop(suspender.install, self._suspension)
    ...
    def remove_suspender(self, suspender):
        ...
        if suspender in self._suspenders:
            self._on_loop(suspender.remove)
    ...
    def _arrange_suspension(self):
        """Hold the plan if a condition is already bad, and watch from here.

        Called as a plan starts and again as one resumes, because both are
        entries into a running plan from a state where the user had control and
        the suspension may have moved with nothing watching it.

        One read decides both halves, so they cannot disagree: a condition
        tripping between "is it tripped" and "start watching" used to leave the
        plan with nothing holding it and a watcher that believed it was already
        held.
        """
        tripped = self._suspension.tripped
        if tripped and self._cleared_when_paused:
            justifications = join_justifications(self._suspension.reasons)
            print(
            ...
            # Held in band, ahead of the plan's first message. A suspension
            # proper cannot do this job: there is no checkpoint yet to rewind
            # to, so asking for one would abort the plan rather than hold it.
            self._plan_stack.append(single_gen(Msg("wait_for", None, [self._suspension.wait_cleared])))
            self._response_stack.append(None)

        # A task belongs to the loop that made it, and this is called from
        # whatever thread started or resumed the plan, so the making crosses.
        def start_supervisor():
            if self._supervisor is not None:
                self._supervisor.cancel()
            self._supervisor = self._loop.create_task(self._supervise_suspension(tripped_at_start=tripped))

        self._on_loop(start_supervisor)

    async def _supervise_suspension(self, tripped_at_start=False):
        """Suspend the plan whenever the suspension is tripped.
        ...
        """
        if tripped_at_start:
            # Already held in band by `_arrange_suspension`, and there is no
        ...
        while True:
            ...
            if not reasons or self._state in ("paused", "pausing"):
            ...
            justification = join_justifications(opening)
            if justification:
                print(f"Justification for this suspension:\n{justification}")
            if not self._begin_suspension(opening):
                return
            # The plan runs the episode from here -- taking in whatever joins
            # it -- so this only waits for it to end before another can open.
            await self._suspension.wait_cleared()

    def _begin_suspension(self, opening):
        ...
        self._plan_stack.append(single_gen(Msg("_start_suspender", None, opening)))
        self._response_stack.append(None)
        ...
        # replays it onto the message just pushed.
    ...
    async def _start_suspender(self, msg):
        ...
        # accumulator in `msg.args` would be shared with any replay.
        joined = []
        ...
        try:
            ...
            while True:
                ...
                if not self._run_permit.is_set():
                    ...
                    # it was not, anything tripped by then arrived during the
            ...
            # The supervisor watches the suspension on behalf of *this* plan,
            # and waits forever when nothing is tripped, so it has to be told
            # the plan is over. `_arrange_suspension` starts a new one for the
            # next plan, and for a resume of this one.
            if self._supervisor is not None:
                self._supervisor.cancel()
...
def _called(plan):
    """A plan, or the result of calling one that is a callable."""
