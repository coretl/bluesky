...
class RunEngineResult:
    ...
    exception :
        The exception thrown in to end the plan early, if any. An instance:
        `~bluesky.utils.RequestStop` for ``stop``, `~bluesky.utils.RequestAbort`
        for ``abort``, `~bluesky.utils.PlanHalt` for ``halt``.
    """
...
class RunEngine:
    ...
    @property
    def state(self):
        # 'panicked' is the RunEngine's own state, not the plan's: it means
    ...
    def commands(self):
        ...
        >>> RE.commands
        """
        # The session answers with the docstrings too; this has always been the
        # names alone, as a list, in the order the commands were registered.
    ...
    @property
    def rewindable(self):
        # While a plan is in flight this is the plan's live value, which it may
        # have changed for its own duration. Otherwise it is the session's
    ...
    @property
    def suspenders(self):
        """Every suspender that can suspend the plan in progress.
    ...
    # Emission belongs to the dispatcher of the runner running the plan, which
    # lets the chain carry the document to the session's subscribers too. The
    # runner offers no `emit` of its own: this is the only caller, and one
    # method existing to be forwarded to is not an API.
    def emit(self, name, doc):
        """Give a document to every subscriber."""
        self._runner._dispatcher.process(name, doc)
